...
- **MCP `watch_video` synchronous tool: still blocking, no longer piped.** Wedged the host on Claude Desktop / Windows in v2.0.x from a stdio pipe-buffer interaction. ✅ **Fixed in v2.1.0** by the `watch_video_start` + `watch_video_status` polling pair. In v2.5.0 the blocking tool itself was moved onto the same log-file-backed runner: subprocess output is no longer piped, so there's no per-step drain latency and no orchestrator-side deadlock at that layer. It still holds one MCP tool call open for the whole pipeline though, and a host that penalizes long-running tool calls can still stall on that, independent of the piping fix. `watch_video_start` + `watch_video_status` remain the recommended pattern there. v2.5.1 moves the one-shot tools (`fetch_jira_attachment`, `post_to_jira`, `pick_highlights`) off the pipe path too: they wedged the same way, and an attachment fetch that takes 2.0 s from a shell never returned in 1800 s under the host. Residual, not fixed there: a cancelled call kills only the direct child, not its grandchildren (ffmpeg, tesseract, whisper), so a leftover process can still hold `frames/` and the step logs while a retry starts a second run in the same workdir; `_reset_run_logs` swallows the failed unlink, and the leftover's newer `ts` then wins in `_read_last_event`. Needs a Windows job object / `taskkill /F /T` and POSIX `start_new_session` + `killpg`, plus failing a run whose logs cannot be cleared.
